...
class SerialGUIApp:
    def __init__(self, root, port="COM5", baud=115200, timeout=1.0, reconnect_delay=2.0):
        ...
        # Filter parameters (now supports multiple simultaneous filters)
        self.filter_lowpass = tk.BooleanVar(value=False)
        self.filter_highpass = tk.BooleanVar(value=False)
        self.filter_bandpass = tk.BooleanVar(value=False)
        self.filter_notch_enabled = tk.BooleanVar(value=False)
        
        self.filter_order = tk.IntVar(value=4)
        self.filter_low = tk.DoubleVar(value=1.0)
        self.filter_high = tk.DoubleVar(value=40.0)
        self.filter_notch_freq = tk.DoubleVar(value=50.0)
    ...
    def _build_filter_controls(self, parent):
        """Signal filtering controls (multiple filters can be applied simultaneously)"""
        frame = ttk.LabelFrame(parent, text="Filtering")
        frame.pack(fill=tk.X, padx=5, pady=5)
        
        # Filter type checkboxes (multiple selection)
        ttk.Label(frame, text="Enable:").grid(row=0, column=0, columnspan=2, sticky=tk.W, padx=5, pady=(3, 1))
        
        lowpass_check = ttk.Checkbutton(frame, text="Lowpass", variable=self.filter_lowpass, command=self._draw_graph)
        lowpass_check.grid(row=1, column=0, sticky=tk.W, padx=5, pady=1)
        
        highpass_check = ttk.Checkbutton(frame, text="Highpass", variable=self.filter_highpass, command=self._draw_graph)
        highpass_check.grid(row=1, column=1, sticky=tk.W, padx=5, pady=1)
        
        bandpass_check = ttk.Checkbutton(frame, text="Bandpass", variable=self.filter_bandpass, command=self._draw_graph)
        bandpass_check.grid(row=2, column=0, sticky=tk.W, padx=5, pady=1)
        
        notch_check = ttk.Checkbutton(frame, text="Notch", variable=self.filter_notch_enabled, command=self._draw_graph)
        notch_check.grid(row=2, column=1, sticky=tk.W, padx=5, pady=1)
        
        # Order
        ttk.Label(frame, text="Order:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=3)
        order_spin = ttk.Spinbox(frame, from_=1, to=10, textvariable=self.filter_order, width=20)
        order_spin.grid(row=3, column=1, sticky=tk.EW, padx=5, pady=3)
        try:
        ...
        # Low frequency
        ttk.Label(frame, text="Low (Hz):").grid(row=4, column=0, sticky=tk.W, padx=5, pady=3)
        low_entry = ttk.Entry(frame, textvariable=self.filter_low, width=20)
        low_entry.grid(row=4, column=1, sticky=tk.EW, padx=5, pady=3)
        low_entry.bind('<Return>', lambda e: self._draw_graph())
        low_entry.bind('<FocusOut>', lambda e: self._draw_graph())
        
        # High frequency
        ttk.Label(frame, text="High (Hz):").grid(row=5, column=0, sticky=tk.W, padx=5, pady=3)
        high_entry = ttk.Entry(frame, textvariable=self.filter_high, width=20)
        high_entry.grid(row=5, column=1, sticky=tk.EW, padx=5, pady=3)
        high_entry.bind('<Return>', lambda e: self._draw_graph())
        high_entry.bind('<FocusOut>', lambda e: self._draw_graph())
        
        # Notch frequency
        ttk.Label(frame, text="Notch (Hz):").grid(row=6, column=0, sticky=tk.W, padx=5, pady=3)
        notch_entry = ttk.Entry(frame, textvariable=self.filter_notch_freq, width=20)
        notch_entry.grid(row=6, column=1, sticky=tk.EW, padx=5, pady=3)
        ...
        normalize_check.grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=3)
        
        detrend_check = ttk.Checkbutton(frame, text="Detrend", variable=self.detrend, command=self._draw_graph)
        detrend_check.grid(row=8, column=0, columnspan=2, sticky=tk.W, padx=5, pady=3)
    ...
    def _apply_processing(self, data, fs):
        ...
        # Apply filters (multiple simultaneous filters in sequence)
        try:
            order = self.filter_order.get()
            
            # Lowpass filter
            if self.filter_lowpass.get():
                try:
                    ...
                    if 0 < normalized_freq < 1:
                        ...
                        if len(d) > padlen:
                            ...
                            self._log(f"Lowpass skipped: need >{padlen} samples, have {len(d)}")
                except Exception as e:
                    self._log(f"Lowpass filter error: {e}")
            
            # Highpass filter
            if self.filter_highpass.get():
                try:
                    ...
                    if 0 < normalized_freq < 1:
                        ...
                        if len(d) > padlen:
                            ...
                            self._log(f"Highpass skipped: need >{padlen} samples, have {len(d)}")
                except Exception as e:
                    self._log(f"Highpass filter error: {e}")
            
            # Bandpass filter
            if self.filter_bandpass.get():
                try:
                    ...
                    if 0 < low < high < nyquist:
                        ...
                        if len(d) > padlen:
                            ...
                            self._log(f"Bandpass skipped: need >{padlen} samples, have {len(d)}")
                except Exception as e:
                    self._log(f"Bandpass filter error: {e}")
            
            # Notch filter
            if self.filter_notch_enabled.get():
                try:
                    freq = self.filter_notch_freq.get()
                    Q = 30
                    ...
                    self._log(f"Notch filter error: {e}")
        except Exception as e:
